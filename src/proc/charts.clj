...
(defn root->charts [root interests phases group-key subs subints]
...
(defn charts->dwells [charts]
  (remove #(nil? (.getDataset (.getPlot %))) (all-chart-types :dwell charts)))

(defn charts->fills [charts]
  (all-chart-types :fill charts))

(defn ->set-bounds [charts xlow xhigh ylow yhigh]
...
(defn ->sync [charts syncys xlow xhigh ylow yhigh]
  (doseq [k [:dwell :fill]]
...
(defn ->lines [charts phase-lines phstarts]
...
(defn ->save-dwell-fill [root charts save-dwell save-fill]
